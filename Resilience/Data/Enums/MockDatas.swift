...
enum MockDatas {
	...
	static let plants: [Plant] = [
		Plant(
			...
			bloomingPeriods: [
				(
				start: Calendar.current.date(from: DateComponents(month: 1, day: 1)) ?? Date(),
				end: Calendar.current.date(from: DateComponents(month: 1, day: 15)) ?? Date()
			), 
				(
				start: Calendar.current.date(from: DateComponents(month: 6, day: 1)) ?? Date(),
				end: Calendar.current.date(from: DateComponents(month: 6, day: 5)) ?? Date()
			)
			],
			fruitingPeriods: [
				(
				start: Calendar.current.date(from: DateComponents(month: 8, day: 1)) ?? Date(),
				end: Calendar.current.date(from: DateComponents(month: 8, day: 31)) ?? Date()
			)
			],
			hardiness: -20,
